...
//    const double input =
//        i > largeWindow && i <= 2 * largeWindow ? amplitude : 0.0;
//    smallAverage.addInput(input);
//    largeAverage.addInput(input);
//    double setAvg = set.addInputGetMax(input, 0.0);
//    if (i % printInterval == 0) {
//      printf("[%5zu] input=%8.3lf ; avg1=%18.16lf ; avg2=%18.16lf ; "
//             "set1=%18.16lf ; set2=%18.16lf ; setMax=%18.16lf\n",
//             i, input, smallAverage.getAverage(), largeAverage.getAverage(),
//             set.getAverage(0), set.getAverage(1), setAvg);
//    }
//  }
//}
//
...
//      input[i] =
//          THRESHOLD * (1.0 + sin(M_2_PI * i / PERIODS) * rand() / RAND_MAX);
//      if (i % RANDOM_PEAK == 0) {
//        input[i] += 50;
//      }
//    }
...
//        input[i] = flip ? 70 : 80;
//      } else {
//        input[i] = THRESHOLD * (1.0 + sin(M_2_PI * i / PERIODS));
//      }
//      peak1--;
//      peak2--;
//    }
...
//}
//
//template <typename T, bool IS_NUMBER> struct Zero;
//
//template <typename T> struct Zero<T, false> {
//  static const T zero() {
//    T z;
//    return z;
//  }
//};
//
//template <typename T> struct Zero<T, true> {
...
//  }
//
//  void ensureCapacityForIndex(size_t index) { ensureCapacity(index + 1); }
//
//public:
...
//  double out2 = 0;
//
//  void print() const { printf("%zi   %lf   %lf   %lf\n", t, in, out1, out2); }
//};
//
//static void printMeasurement(const Measurement m) { m.print(); }
//
...
//    integration.integrate(mem2, mem3);
//    integration.integrate(mem3, mem4);
//    printf("# \tin=%lf, mem1=%lf, mem2=%lf\n", input, mem1, mem2);
//  }
//
//  printf("# Double integration of linear [0..1] slope with %zu times "
//         "integration time yields %lf/%lf\n",
//         MUL, mem2, 1 / mem2);
//}
//
//static void integrationTimes() {
//  static constexpr size_t RC = 16;
//  static constexpr size_t MUL = 6;
//  static constexpr size_t COUNT = RC * MUL;
//
//  IntegrationCoefficients<double> integration(RC);
//  double mem1 = 0.0;
...
//
//  for (size_t i = 1; i <= COUNT; i++) {
//    integration.integrate(1.0, mem1);
//    integration.integrate(mem1, mem2);
//    integration.integrate(mem2, mem3);
//    integration.integrate(mem3, mem4);
//    printf("# \tint-1=%lf, int-2=%lf, int-3=%lf, int-4=%lf\n", mem1, mem2, mem3, mem4);
//  }
//}
//
...
//
//static void
//printDelayEntry(const typename MultiChannelAndTimeDelay<int>::Entry &entry) {
//  cout << "\tdelay=" << entry.delay_ << "; end=" << entry.end_
//       << "; write=" << entry.write_ << "; read=" << entry.read_ << endl;
//}
//
//static void printDelayState(const MultiChannelAndTimeDelay<int> &delay) {
//  cout << "Delay channels=" << delay.channels_
//       << "; max-channels=" << delay.maxChannels_ << endl;
...
//
//  for (int i = 0; i < 100; i++) {
//    cout << "channel 0: getAndSet(" << (i + 100)
//         << ") = " << delay.setAndGet(0, i + 100) << endl;
//    cout << "channel 1: getAndSet(" << (i + 200)
//         << ") = " << delay.setAndGet(1, i + 200) << endl;
//    cout << "channel 2: getAndSet(" << (i + 300)
//         << ") = " << delay.setAndGet(2, i + 300) << endl;
//    cout << "channel 3: getAndSet(" << (i + 400)
//         << ") = " << delay.setAndGet(3, i + 400) << endl;
//    printDelayState(delay);
//    delay.next();
//  }
//}
