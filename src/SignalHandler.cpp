...
public:
  ...
  bool is_acive() const { return id_ > 0; }
...
class thread_entries {
  ...
  bool is_active_thread_number(size_t number) const {
    ...
    for (size_t i = 0;
         i < MAX_THREAD_ENTRIES && active_count < active_thread_count; i++) {
...
public:
  size_t add_thread(const char *name) {
    ...
    if (active_thread_count < MAX_THREAD_ENTRIES) {
      ...
      for (size_t slot_number = 0; slot_number < MAX_THREAD_ENTRIES;
           slot_number++) {
  ...
  bool await_and_report(std::chrono::milliseconds timeout,
                        const char *start_wait_message) {
...
const char *signal_exception::what() const { return message_; }

void signal_exception::handle() const { handle(nullptr); }
...
SignalHandler::SignalHandler() { ensure_handlers(); }
...
bool SignalHandler::int_is_set() const { return int_get_signal() != -1; }
...
int SignalHandler::get_signal() { return instance().int_get_signal(); }

bool SignalHandler::is_set() { return instance().int_is_set(); }

int SignalHandler::raise_signal(int signal) {
  return instance().int_raise_signal(signal);
}

bool SignalHandler::check_raised() { return instance().int_check_raised(); }
