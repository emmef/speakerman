...
int socket_input_stream::unsafe_read() {
  ...
  int r = blocking_ ? recv(file_descriptor(), buffer_, STREAM_BUFFER_SIZE, 0)
                    : recv(file_descriptor(), buffer_, STREAM_BUFFER_SIZE,
                           MSG_DONTWAIT);
  if (r < 0) {
    switch (errno) {
    case EAGAIN:
...
void socket_input_stream::set_bocking(bool value) { blocking_ = value; }

bool socket_input_stream::get_bocking() { return blocking_; }
...
void socket_input_stream::close() { set_file(-1, 0); }

void socket_input_stream::on_file_set() { pos_ = mark_ = 0; }

void socket_input_stream::close_file() { ::close(file_descriptor()); }

void socket_input_stream::before_close_file() {}

socket_input_stream::~socket_input_stream() { close(); }

int socket_output_stream::unsafe_send(size_t offset, size_t count,
                                      bool do_throw) {
...
socket_output_stream::socket_output_stream()
    : socket_output_stream(-1, false) {}

void socket_output_stream::on_file_set() { pos_ = 0; }
...
void socket_output_stream::close() { set_file(-1, false); }

void socket_output_stream::close_file() { ::close(file_descriptor()); }

void socket_output_stream::before_close_file() { flush(); }
