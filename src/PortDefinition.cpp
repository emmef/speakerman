...
namespace speakerman {
    ...
    PortDefinition::PortDefinition(const PortDefinition::Data source) : data(validated(source))
    {};

    PortDefinition::PortDefinition(const char *name, PortDirection direction, PortIsTerminal terminal) :
    ...
    PortDefinitions::PortDefinitions(size_t maxPorts, size_t nameStorageSize) :
            definitions(maxPorts, 0), nameStorage(
            validSize(nameStorageSize, maxPorts), 0)
    {
    }
    ...
    PortDefinitions::PortDefinitions(const PortDefinitions &source,
            ...
            definitions(source.definitions, policy), nameStorage(source.nameStorage,
                                                                 policy)
    {
    ...
    int PortDefinitions::indexOf(const char *name) const
    {
        for (size_t i = 0; i < definitions.size(); i++) {
            if (strncasecmp(name, definitions[i].name, Names::get_port_size())
                == 0) {
                return i;
            }
        }
    ...
    int PortDefinitions::indexOf(const char *name,
    ...
    const speakerman::PortDefinition::Data PortDefinitions::getByName(
    ...
    const speakerman::PortDefinition::Data *PortDefinitions::getByNamePtr(
            const char *name) const
    {
        int idx = indexOf(name);
    ...
    const speakerman::PortDefinition::Data &PortDefinitions::operator[](
            size_t index) const
    {
        return definitions[index];
    }

    const speakerman::PortDefinition PortDefinitions::operator()(
            size_t index) const
    {
