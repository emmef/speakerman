...
namespace speakerman {
    ...
    {
        static const size_t value = 2 + // Begin and end markers ^$
                                                 ...
                                                 size_t portLength)
    {
        ...
        if (clientLength > 0 && portLength > 0) {
            checkLengthOrThrow(SIZE,
                               snprintf(formatTemplate, SIZE, "^%s%s%s$",
                                        template_name_regex(), client_port_separator(),
                                        template_name_regex()));
        }
        else {
            checkLengthOrThrow(SIZE,
                               snprintf(formatTemplate, SIZE, "^%s$", template_name_regex()));
        }
        if (clientLength > 0) {
            if (portLength > 0) {
                checkLengthOrThrow(SIZE,
                                   snprintf(pattern, SIZE, formatTemplate, minimum_name_length,
                                            clientLength, minimum_name_length, portLength));
            }
            else {
                checkLengthOrThrow(SIZE,
                                   snprintf(pattern, SIZE, formatTemplate, minimum_name_length,
    ...
    const char *const Names::client_port_separator()
    {
        return ":";
    }
    ...
    {
        static const size_t value = get_full_size() - get_client_size()
    ...
    size_t NameListPolicy::maxNames() const
    {
        return Count<const char *>::max();
    }

    size_t NameListPolicy::maxCharacters() const
    {
    ...
    NameList::NameList(const NameListPolicy &policy,
                       size_t initialNameCapacity, size_t initialCharacterCapacity) :
            nameCount_(0), nameCapacity_(initialNameCapacity), names_(
            initialNameCapacity > 0 ?
            new const char *[initialNameCapacity] : nullptr), characterCount_(
            0), characterCapacity_(initialCharacterCapacity), characters_(
            initialCharacterCapacity > 0 ?
            new char[initialCharacterCapacity] : nullptr), policy_(
            policy)
    {
    }

    NameList::NameList(const NameListPolicy &policy) :
            NameList(policy, 16, 1024)
    {
    }

    NameList::NameList(NameList &&source) :
            nameCount_(source.nameCount_), nameCapacity_(source.nameCapacity_), names_(
            source.names_), characterCount_(source.characterCount_), characterCapacity_(
            source.characterCapacity_), characters_(source.characters_), policy_(
            source.policy_)
    {
        source.removeAll();
    ...
    NameList::NameList(const NameList &source) :
            nameCount_(source.nameCount_), nameCapacity_(source.nameCapacity_), names_(
            nameCapacity_ > 0 ? new const char *[nameCapacity_] : nullptr), characterCount_(
            source.characterCount_), characterCapacity_(
            source.characterCapacity_), characters_(
            characterCapacity_ > 0 ?
            new char[characterCapacity_] : nullptr), policy_(
            source.policy_)
    {
    ...
    const char *NameList::operator[](size_t i) const
    {
        return get(i);
    }
    ...
    size_t PortNames::countPorts(const char **portNames,
    ...
    PortNames::PortNames(const char **names, FreeNames free,
                         size_t maxSensibleNames) :
            portNames_(names), count_(countPorts(portNames_, maxSensibleNames)), free_(
            free)
    {
    }

    PortNames::PortNames(PortNames &&source) :
            portNames_(source.portNames_), count_(source.count_), free_(
            source.free_)
    {
