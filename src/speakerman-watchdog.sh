...
exceeded_allowed() {
  ...
  while read line; do
    ...
    if echo "$line" | egrep "$expr_nodashes" >/dev/null; then
    ...
    elif echo "$line" | egrep "$expr_dashes" >/dev/null; then
    ...
    if echo "$startstamp-$endstamp:$level" | egrep '^[0-9]{12}-[0-9]{12}:(1|2|3|4|5|6)$' >/dev/null; then
      if test "$now" -ge "$startstamp" && test "$now" -le "$endstamp"; then
