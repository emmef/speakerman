...
using namespace std;
	...
	class Transport
	{
		static_assert(is_trivially_copyable<Data>::value, "Expected Data parameter that is trivial to copy");
	...
	public:
		...
		class LockFreeData
		{
			...
			LockFreeData(Data &data, const bool write, volatile bool &read, bool useFence) :
					data_(&data)
			{
				write_ = write;
				read_ = &read;
		...
		{
			MemoryFence fence;
			// volatile only prevents reorder, does not guarantee inter-thread visibility
			shutdown_ = true;
		}
	};
