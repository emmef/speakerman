#ifndef TDAP_M_DEBUG_HPP
#define TDAP_M_DEBUG_HPP
/*
 * tdap/debug.hpp
 *
 * Part of TdAP
...
#define TDAP_DEBUG_DEF_COUNT(name)                                             \
  ...
  static inline void debug##name##Call() {                                     \
    static const int delta =                                                   \
        debugArrayRegisterCount(true, debug##name##Count);                     \
    debug##name##Count += delta;                                               \
    std::cout << "\t" << #name << "(" << debug##name##Count << ")"             \
              << std::endl;                                                    \
  }                                                                            \
  static inline void debug##name##Zero() { debug##name##Count = 0; }
#else
#define TDAP_DEBUG_DEF_COUNT(name)                                             \
  static int debug##name##Count = 0;                                           \
  static inline void debug##name##Call() {                                     \
