...
struct Integration {
	...
	static F valid_samples(double sampleRate, double seconds)
	{
		...
		throw std::invalid_argument("Average:: Combination of sampleRate_ and seconds yields too large sample count");
	}
};
...
public:
	...
	{
		static_assert(std::is_floating_point<Value>::value, "Value must be a floating-point type (stability condition)");
...
struct AttackReleaseSmoothFilter
{
	...
	AttackReleaseSmoothFilter(Coefficient attackSamples, Coefficient releaseSamples, Coefficient initialOutput = 0.0) :
