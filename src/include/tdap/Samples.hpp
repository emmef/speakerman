...
namespace tdap {
    ...
    struct alignas(ALIGN) Samples : public FixedSizeArray<T, SIZE> {
        static_assert(
                std::is_arithmetic<T>::value && std::is_scalar<T>::value,
            ...
            HelperVectorTraits<T, ROWS, COLUMNS, SampleMatrix, COLUMNS == ROWS>
    {
        ...
        void multiply_in(Samples<T, ROWS, AL1> &output, const Samples<T, COLUMNS, AL2>&input) const
        {
            for (size_t row = 0; row < ROWS; row++) {
        ...
        void multiply_in(SampleMatrix<T, ROWS, N, AL1> &output, const SampleMatrix<T, COLUMNS, N, AL2>&input) const
        {
            for (size_t row = 0; row < ROWS; row++) {
