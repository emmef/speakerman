    ...
    struct ACurves
    {
        ...
        struct SingleParametricBestFit
        {
            ...
            public:
                ...
                double relativeFrequency() const
                { return 1.0 / period_; }
            ...
            { return hz_top.frequency * SAMPLERATE_SCALE; }

            static constexpr double relativeFrequency(double f)
            { return f / sampleRate(); }
            ...
            void measure(double gains[COUNT])
            {
                ...
                for (size_t i = 0; i < count; i++) {
                    ...
                    lowPassFilter.filter(UNITY,
                                         highPassFilter.filter(UNITY, paraFilter.filter(UNITY, sin_unity.next())));
                ...
                //measure
                for (size_t i = 0; i < count; i++) {
                    gains[LOW] = Value<double>::max(gains[LOW], fabs(lowPassFilter.filter(LOW,
                                                                                          highPassFilter.filter(LOW,
                                                                                                                paraFilter.filter(
                                                                                                                        LOW,
                                                                                                                        sin_low.next())))));
                    gains[MID] = Value<double>::max(gains[MID], fabs(lowPassFilter.filter(MID,
                                                                                          highPassFilter.filter(MID,
                                                                                                                paraFilter.filter(
                                                                                                                        MID,
                                                                                                                        sin_mid.next())))));
                    gains[UNITY] = Value<double>::max(gains[UNITY], fabs(lowPassFilter.filter(UNITY,
                                                                                              highPassFilter.filter(
                                                                                                      UNITY,
                                                                                                      paraFilter.filter(
                                                                                                              UNITY,
                                                                                                              sin_unity.next())))));
                    gains[TOP] = Value<double>::max(gains[TOP], fabs(lowPassFilter.filter(TOP,
                                                                                          highPassFilter.filter(TOP,
                                                                                                                paraFilter.filter(
                                                                                                                        TOP,
                                                                                                                        sin_top.next())))));
                    gains[HIGH] = Value<double>::max(gains[HIGH], fabs(lowPassFilter.filter(HIGH,
                                                                                            highPassFilter.filter(HIGH,
                                                                                                                  paraFilter.filter(
                                                                                                                          HIGH,
                                                                                                                          sin_high.next())))));
                }
