#ifndef TDAP_M_WEIGHTING_HPP
#define TDAP_M_WEIGHTING_HPP
/*
 * tdap/Weighting.hpp
 *
 * Part of TdAP
...
struct ACurves {
    ...
    if (coeffs.order() != 1) {
      if (coeffs.hasFixedOrder()) {
        throw std::invalid_argument(
            "Coefficient argument cannot be set to order 1");
      }
      coeffs.setOrder(1);
    }
  }
  ...
  static void setCurveParameters(IirCoefficients &coeffs, double sampleRate) {
    BiQuad::setParametric(coeffs, sampleRate, PARAM_CENTER, PARAM_GAIN,
                          PARAM_BANDWIDTH);
  }
  ...
  static void setLowPassParameters(IirCoefficients &coeffs, double sampleRate) {
    ...
    Butterworth::create(coeffs, sampleRate, LOW_PASS_FREQUENCY,
                        Butterworth::Pass::LOW, 1.0);
  }

  static void setHighPassParameters(IirCoefficients &coeffs,
                                    double sampleRate) {
    setFirstOrder(coeffs);
    Butterworth::create(coeffs, sampleRate, HIGH_PASS_FREQUENCY,
                        Butterworth::Pass::HIGH, 1.0);
  }
#endif
  template <typename SAMPLE> class Coefficients {
  ...
  public:
    ...
    Coefficients(double sampleRate) { setSampleRate(sampleRate); }
    ...
    const FixedSizeIirCoefficients<SAMPLE, 2> &curve() const { return curve_; }
  ...
  template <typename SAMPLE, size_t CHANNELS> class Filter {
    ...
    struct SingleChannelFilter : public tdap::Filter<SAMPLE> {
      ...
      virtual SAMPLE filter(SAMPLE input) { return wrapped_.filter(0, input); }

      SingleChannelFilter(Filter<SAMPLE, CHANNELS> &wrapped)
          : wrapped_(wrapped) {}

      SingleChannelFilter(SingleChannelFilter &&source)
          : wrapped_(source.wrapped_) {}
    };
    ...
    struct MultiChannelFilter : public tdap::MultiFilter<SAMPLE> {
      ...
      MultiChannelFilter(MultiChannelFilter &&source)
          : wrapped_(source.wrapped_) {}
    };
  ...
  public:
    ...
    Filter(const Coefficients<SAMPLE> &coefficients)
    ...
    template <bool flushToZero> SAMPLE do_filter(size_t channel, SAMPLE input) {
      IndexPolicy::array(channel, CHANNELS);
      SAMPLE y = input;
      y = coefficients_.curve().template do_filter<SAMPLE, flushToZero>(
          history_[channel].curveX, history_[channel].curveY, y);
#ifdef TDAP_FULL_ACURVE
      y = coefficients_.lowPass().template do_filter<SAMPLE, flushToZero>(
          history_[channel].lowX, history_[channel].lowY, y);
      y = coefficients_.highPass().template do_filter<SAMPLE, flushToZero>(
          history_[channel].highX, history_[channel].highY, y);
#endif
    ...
    template <typename... A>
    void filterArray(const ArrayTraits<SAMPLE, A...> &input,
                     ArrayTraits<SAMPLE, A...> &output) {
