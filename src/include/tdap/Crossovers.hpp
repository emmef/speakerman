#ifndef TDAP_M_CROSSOVERS_HPP
#define TDAP_M_CROSSOVERS_HPP
/*
 * tdap/Crossovers.hpp
 *
 * Part of TdAP
...
struct Crossovers {
  ...
  template <typename T, size_t CHANNELS> struct LinkwitzRiley {
    ...
    void configure(T sampleRate, T frequency) {
      ...
      Butterworth::create(wrapLo, sampleRate, frequency, Butterworth::Pass::LOW,
                          1.0);
      auto wrapHi = highPass.coefficients_.wrap();
      Butterworth::create(wrapHi, sampleRate, frequency,
                          Butterworth::Pass::HIGH, 1.0);
      lowPass.reset();
      highPass.reset();
    }
  };
  ...
  struct CrossoverExecutor<T, CHANNELS, 1> {
    ...
    static void filter(const FixedSizeArrayTraits<S, CHANNELS, A...> &input,
      ...
      for (size_t channel = 0, idx = 0; channel < CHANNELS;
           channel++, idx += 2) {
        T in = input[channel];
        output[channel] = filter[0].lowPass.filter(
            idx, filter[0].lowPass.filter(idx + 1, in));
        output[channel + CHANNELS] = filter[0].highPass.filter(
            idx, filter[0].highPass.filter(idx + 1, in));
      }
    }
  };
  ...
  struct CrossoverExecutor<T, CHANNELS, 2> {
    ...
    static void filter(const FixedSizeArrayTraits<S, CHANNELS, A...> &input,
      ...
      for (size_t channel = 0, idx = 0; channel < CHANNELS;
           channel++, idx += 2) {
        T in = input[channel];
        T middle = filter[1].lowPass.filter(
            idx, filter[1].lowPass.filter(idx + 1, in));
        output[channel + 2 * CHANNELS] = filter[1].highPass.filter(
            idx, filter[1].highPass.filter(idx + 1, in));
        output[channel] = filter[0].lowPass.filter(
            idx, filter[0].lowPass.filter(idx + 1, middle));
        output[channel + CHANNELS] = filter[0].highPass.filter(
            idx, filter[0].highPass.filter(idx + 1, middle));
      }
    }
  };
  ...
  struct CrossoverExecutor<T, CHANNELS, 3> {
    ...
    static void filter(const FixedSizeArrayTraits<S, CHANNELS, A...> &input,
      ...
      for (size_t channel = 0, idx = 0; channel < CHANNELS;
           channel++, idx += 2) {
        T in = input[channel];
        T low = filter[1].lowPass.filter(idx,
                                         filter[1].lowPass.filter(idx + 1, in));
        T high = filter[1].highPass.filter(
            idx, filter[1].highPass.filter(idx + 1, in));

        output[channel + 3 * CHANNELS] = filter[2].highPass.filter(
            idx, filter[2].highPass.filter(idx + 1, high));
        output[channel + 2 * CHANNELS] = filter[2].lowPass.filter(
            idx, filter[2].lowPass.filter(idx + 1, high));
        output[channel + CHANNELS] = filter[0].highPass.filter(
            idx, filter[0].highPass.filter(idx + 1, low));
        output[channel] = filter[0].lowPass.filter(
            idx, filter[0].lowPass.filter(idx + 1, low));
      }
    }
  };
  ...
  public:
    ...
    void configure(S1 sampleRate, const ArrayTraits<S2, A...> &crossovers) {
      FixedSizeArray<T, CROSSOVERS> frequencies =
          validatedCrossoverFrequencies<T, CROSSOVERS, S2, A...>(crossovers);
  ...
  weights(const FixedSizeArrayTraits<T, CROSSOVERS, A...> &crossovers,
          double sampleRate) {
    ...
    Butterworth::create(llCoeffs, sampleRate, 20.0, Butterworth::Pass::HIGH,
                        1.0);
    FixedSizeIirCoefficientFilter<double, 1, 4> cutoffHigh;
    auto hhCoeffs = cutoffHigh.coefficients_.wrap();
    Butterworth::create(hhCoeffs, sampleRate, 8000.0, Butterworth::Pass::LOW,
                        1.0);
    ...
    for (size_t sample = 0; sample < samples; sample++) {
      ...
      T input = cutoffHigh.filter(
          0, cutoffLow.filter(0, noiseValue)); // bandwidth limited pink noise
    ...
    for (size_t band = 0; band < 2 * CROSSOVERS + 2; band++) {
      ...
      y[band] =
          sqrtRelativeWeight; // correct with unweighted full-range measurement
    }
