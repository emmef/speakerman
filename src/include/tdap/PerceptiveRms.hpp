...
namespace tdap {
    ...
    /**
     * Defines a system that can calculate weighted measurements at different integration times
     * according to how loud they are perceived. This can be used to construct perceptive RMS
     * measurements.
     */
    struct Perceptive {
        ...
        struct Metrics {
            ...
            Metrics(size_t count, size_t perceptive, double slowSeconds, double fastSeconds) :
                    count_(std::max(2ul, count)),
                    perceptive_(std::min(perceptive, count_ - 2ul)),
                    slowSeconds_(std::clamp(
                                      slowSeconds,
                                      PERCEPTIVE_SECONDS * pow(MIN_STEP_FACTOR, perceptive_),
                                      MAX_SLOW_SECONDS)),
                    fastSeconds_(std::clamp(fastSeconds, MIN_FAST_SECONDS, MAX_FAST_SECONDS)) {}
            ...
            [[nodiscard]] size_t fastSteps() const noexcept { return fastest() - perceptive_; }
            ...
            [[nodiscard]] static Metrics createWithEvenSteps(
                    double slowSeconds,
                    double fastSeconds,
                    size_t maxLevels) {
              ...
              double maxSlowSteps = (log(slow) - log(PERCEPTIVE_SECONDS)) / log(MIN_STEP_FACTOR);
              double fast = clamp(fastSeconds, MIN_FAST_SECONDS, MAX_FAST_SECONDS);
              double maxFastSteps = (log(PERCEPTIVE_SECONDS) - log(fast)) / log(MIN_STEP_FACTOR);
...
static std::ostream &operator << (std::ostream &stream, const tdap::Perceptive::Metrics& metrics) {
  stream << "Perceptive::Metrics:" << std::endl;
  static constexpr size_t LENGTH = 32;
  char format[LENGTH];
  for (size_t i = 0; i < metrics.count(); i++) {
      snprintf(format, LENGTH, " %2zu.%2s%7.4lfs weight %3zu%%",
              i, ((i == metrics.perceptive()) ? "*" : " "),
              metrics.seconds(i), size_t(metrics.weight(i) * 100));
...
namespace tdap {
    ...
    public:
        ...
        void configure(size_t sample_rate,
                       const Perceptive::Metrics &metrics,
                       S initial_value = 0.0)
        {
          rms_.setUsedWindows(metrics.count());
          for (size_t i = 0; i < metrics.count(); i++) {
            double weight = metrics.weight(i);
            rms_.setWindowSizeAndScale(i, 0.5 + sample_rate * metrics.seconds(i), weight * weight);
          }
          ...
          rms_.setAverages(initial_value);
            ...
            follower_ = SmoothHoldMaxAttackRelease<S>(
                    ...
                    metrics.releaseSeconds() * sample_rate,
                    10);
        }
    ...
    template<typename S, size_t LEVELS>
    class PerceptivePseudoRms {
    ...
    public:
        void configure(double sample_rate,
            ...
            for (size_t i = 0; i < metrics_.count(); i++) {
                ...
                coefficients_[i].setCharacteristicSamples(
                        0.5 * metrics_.seconds(i) * sample_rate);
            ...
            follower_ = SmoothHoldMaxAttackRelease<S>(
                    ...
                    metrics_.releaseSeconds() * sample_rate,
                    10);
        }
        ...
        S add_square_get_unsmoothed(S square, S minimum = 0)
        {
            ...
            for (size_t i = 0; i < metrics_.count(); i++) {
                squareDetect = std::max(
                        squaredWeight_[i] * coefficients_[i].integrate(square, integration_[i]),
                        squareDetect);
            }
