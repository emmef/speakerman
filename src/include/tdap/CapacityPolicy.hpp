...
namespace tdap {
	...
	{
	public:
		static size_t defaultNewCapacity(size_t currentCapacity, size_t currentSize, size_t neededSize)
		{
			size_t capacity = Value<size_t>::max(Value<size_t>::max(currentCapacity, 1) * 3 / 2, neededSize, 16);
			if (capacity < neededSize) {
				throw std::invalid_argument("Size overflow in calculating new capacity");
			}
			return capacity;
		}
		virtual size_t calculateNewCapacity(size_t currentCapacity, size_t currentSize, size_t neededSize) const
		{
		...
		template <typename T>
		void ensureCapacity(T *&data, size_t &capacity, size_t &count, size_t neededCount, size_t maxCapacity) const
		{
			static_assert(std::is_trivially_copyable<T>::value, "List must contain data that can be copied trivially");
