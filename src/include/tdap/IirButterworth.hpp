...
struct Butterworth {
	...
	static constexpr size_t getMaxOrder()
	{
		return 20;
	}
	...
	static Coefficient getHighPassGain(const Freq inputFrequency, const Freq filterFrequency, const size_t order)
	{
	...
	template <typename Coefficient, typename Freq>
	static Coefficient getLowPassGain(Freq inputFrequency, Freq filterFrequency, size_t order)
	{
		if (filterFrequency < Value<double>::min_positive()) {
			return 0.0;
		}
			...
			double relativeFrequency, Coefficient scale = 1.0)
	{
		...
		double scaleOfC = scale * getHighPassScalingFactor(order, relativeFrequency);
	...
	static void getDCoefficients(int order, double relativeFrequency, IirCoefficients &d_coefficients) {
		...
		for(int i = 0; i < order; ++i ) {
			for(int j = i; j > 0; --j ) {
				...
				dcof[2*j+1] += binomials[2*i] * dcof[2*(j-1)+1] + binomials[2*i+1] * dcof[2*(j-1)];
			}
		...
		for (int i = 0; i <= order; i++) {
			 ...
			 * where they where subtracted instead of added. We do adds only, so we need
			 * to negate them here.
			 */
