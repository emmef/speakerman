...
namespace tdap {
    ...
    public:
        ...
        static_assert(Values::is_between(BUCKETS, MINIMUM_BUCKETS, MAXIMUM_BUCKETS),
    ...
    public:
        ...
        size_t setSmallWindowAndRc(size_t newSize, double smallRcIntegrationBuckets, double largeRcIntegrationBuckets)
        {
            size_t proposal1 = Values::force_between(newSize, BUCKETS, (const size_t) (1e6 * BUCKETS));
            samplesPerBucket = newSize / BUCKETS;
            size_t minIntSamples =
                    Value<double>::force_between(smallRcIntegrationBuckets, 2, BUCKETS) * samplesPerBucket;
            size_t maxIntSamples =
                    Value<double>::force_between(largeRcIntegrationBuckets, 2, BUCKETS) * samplesPerBucket *
                    (1L << (LEVELS - 1));
