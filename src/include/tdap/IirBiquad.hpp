...
namespace tdap {
    ...
    struct BiQuad
    {
        ...
        {
            return Value<double>::force_between(slope, SHELVE_SLOPE_MINIMUM, SHELVE_SLOPE_MAXIMUM);
        }
        ...
        static void setParametric(IirCoefficients &coefficients, double sampleRate, double centerFrequency, double gain,
                                  double bandwidth)
        {
        ...
        static void setLowShelve(IirCoefficients &coefficients, double sampleRate, double centerFrequency, double gain,
                                 double slope)
        {
        ...
        static void setHighShelve(IirCoefficients &coefficients, double sampleRate, double centerFrequency, double gain,
                                  double slope)
        {
        ...
        static void
        setLowPass(IirCoefficients &coefficients, double sampleRate, double centerFrequency, double bandwidth)
        {
        ...
        static void
        setHighPass(IirCoefficients &coefficients, double sampleRate, double centerFrequency, double bandwidth)
        {
        ...
        static void
        setBandPass(IirCoefficients &coefficients, double sampleRate, double centerFrequency, double bandwidth)
        {
    ...
    private:
        ...
        static void setCoefficients(IirCoefficients &builder, const BiQuadCoefficients bqc)
        {
            if (builder.order() != 2) {
                if (builder.hasFixedOrder()) {
                    throw std::invalid_argument("Builder must have filter order two (or order can be set)");
                }
                builder.setOrder(2);
            }
        ...
        static BiQuadCoefficients getParametricParameters(
                double relativeCenterFrequency, double gain, double bandwidth)
        {
            ...
            double J = sqrt(gain);
            double g = sw
                       * sinhf(LN_2_2 * limitedBandwidth(bandwidth) * w / sw);
        ...
        static BiQuadCoefficients getLowShelveParameters(
                double relativeCenterFrequency, double gain, double slope)
        {
            double w = Frequency<double>::angularSpeed(relativeCenterFrequency);
            double cw = cosf(w);
            double sw = sinf(w);
        ...
        static BiQuadCoefficients getHighShelveParameters(
                double relativeCenterFrequency, double gain, double slope)
        {
            double w = Frequency<double>::angularSpeed(relativeCenterFrequency);
            double cw = cosf(w);
        ...
        static BiQuadCoefficients getLowPassParameters(
                double relativeCenterFrequency, double bandwidth)
        {
        ...
        static BiQuadCoefficients getHighPassParameters(
                double relativeCenterFrequency, double bandwidth)
        {
            double omega = Frequency<double>::angularSpeed(relativeCenterFrequency);
            double sn = sin(omega);
        ...
        static BiQuadCoefficients getBandPassParameters(
                double relativeCenterFrequency, double bandwidth)
        {
            double omega = Frequency<double>::angularSpeed(relativeCenterFrequency);
            double sn = sin(omega);
