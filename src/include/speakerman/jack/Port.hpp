...
class Port
{
	static int disconnect_port_internal(jack_client_t* client,
			jack_port_t* port, const char* target,
...
public:
	...
	static RefArray<jack_default_audio_sample_t> get_buffer(jack_port_t* port,
			jack_nframes_t frames);
	...
	static bool try_disconnect_port_all(jack_client_t* client,
			jack_port_t* port, int* result = nullptr);
