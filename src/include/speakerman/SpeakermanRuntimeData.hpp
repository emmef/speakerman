...
namespace speakerman {
    ...
    public:
        ...
        const Coefficients &biquad1() const
        { return biquad1_; }
    ...
    public:
        ...
        T limiterScale() const
        { return limiterScale_; }

        T limiterThreshold() const
        { return limiterThreshold_; }

        T signalMeasureFactor() const
        { return signalMeasureFactor_; }
                       ...
                       const ArrayTraits<A...> &relativeBandWeights)
        {
            for (size_t i = 0; i < SpeakermanConfig::MAX_GROUPS; i++) {
                double v = Values::force_between(conf.volume[i], GroupConfig::MIN_VOLUME, GroupConfig::MAX_VOLUME);
    ...
    public:
        GroupRuntimeData<T, BANDS> &groupConfig(size_t i)
        { return groupConfig_[i]; }

        const GroupRuntimeData<T, BANDS> &groupConfig(size_t i) const
        { return groupConfig_[i]; }

        T subLimiterScale() const
        { return subLimiterScale_; }

        T subLimiterThreshold() const
        { return subLimiterThreshold_; }

        T subRmsThreshold() const
        { return subRmsThreshold_; }
    ...
    public:
        ...
        const Data &userSet() const
        { return userSet_; }

        size_t groups() const
        { return GROUPS; }

        size_t channelsPerGroup() const
        { return CHANNELS_PER_GROUP; }
