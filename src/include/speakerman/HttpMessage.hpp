...
struct http_status {
  ...
  static int format_message_extra(char *buffer, size_t max_length,
                                  unsigned status, const char *extraMessage);

  static int format_message_extra(output_stream &stream, unsigned status,
...
class http_message {
  ...
  void write_status(socket_stream &stream, unsigned status,
...
public:
  http_message(size_t buffer_size, size_t headers_size)
      : busy_(false), response_(buffer_size), headers_(headers_size) {}
