...
public:
  StatusCode(int code, const char *message)
      : std::runtime_error(http_status::status_name(code)), code_(code),
        additional_message_(message) {
  ...
  const char *additional_message() const { return additional_message_; }

  bool is_ok() const { return http_status::is_ok(code_); }
};
...
template <typename T> class ResetGuard {
  T *&to_reset_;

public:
  ResetGuard(T *&to_reset) : to_reset_(to_reset) {}

  ~ResetGuard() { to_reset_ = nullptr; }
};
...
bool http_status::is_ok(unsigned status) { return status == OK; }

int http_status::format_message(char *buffer, size_t max_length,
                                unsigned status) {
  return snprintf(buffer, max_length, "HTTP/1.1 %u %s\r\n", status,
                  status_name(status));
}

int http_status::format_message_extra(char *buffer, size_t max_length,
                                      unsigned status,
                                      const char *extraMessage) {
  return snprintf(buffer, max_length, "HTTP/1.1 %u %s\r\n\r\n%s\r\n", status,
                  status_name(status), extraMessage);
}
...
int http_status::format_message_extra(output_stream &stream, unsigned status,
                                      const char *extraMessage) {
...
void http_message::write_status(socket_stream &stream, unsigned status,
...
static int write_header_raw(const char *header, const char *value,
...
unsigned http_message::handle_ok() {
  ...
  if (length > 0) {
    ...
    int r, writes = 0;
    while ((r = response_.read()) >= 0 && (w = stream.write(r)) >= 0) {
      writes++;
    }
    stream.flush();
    return w >= 0 && (r >= 0 || r == stream_result::END_OF_STREAM)
               ? http_status::OK
...
unsigned http_message::handle(socket_stream &stream) {
  ...
  try {
    ...
    std::cerr << "While handling request: status=" << status.code() << " "
              << status.what();
    if (status.has_additional_message()) {
      if (status.code() == http_status::METHOD_NOT_ALLOWED) {
        ...
        http_status::format_message_extra(stream, status.code(),
                                          status.additional_message());
      }
    ...
    http_status::format_message_extra(
        stream, http_status::INTERNAL_SERVER_ERROR, e.what());
    return http_status::INTERNAL_SERVER_ERROR;
  }
}
...
void http_message::set_success() { throw StatusCode(http_status::OK); }

void http_message::set_content_type(const char *type) { content_type_ = type; }

void http_message::handle_content(size_t content_length,
  ...
  if (content_length > 0) {
    ...
    throw StatusCode(http_status::INTERNAL_SERVER_ERROR,
                     "Invalid content stream handle");
  }
}
...
void http_message::read_method() {
  ...
  size_t length = 0;
  for (length = 0;
       length < (read_buffer_size() - 1) && ((c = stream_->read()) >= 0);
       length++) {
    if (c < 'A' || c > 'Z') {
      break;
    }
...
void http_message::read_version() {
  ...
  for (length = 0;
       length < (read_buffer_size() - 1) && ((c = stream.read()) >= 0);) {
...
void http_message::read_headers() {
  ...
  for (length = 0;
       length < read_buffer_length() && ((c = stream_->read()) >= 0);) {
    if (state == NONE) {
      if (c == '\n') {
        state = LF;
      } else if (c == '\r') {
