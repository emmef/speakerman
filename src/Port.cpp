...
int Port::disconnect_port_internal(jack_client_t* client,
		jack_port_t* port, const char* target, bool throw_if_disconnect_fails) {
...
int Port::connect_port_internal(jack_client_t* client, jack_port_t* port,
		const char* target, bool throw_if_disconnect_fails) {
	ErrorHandler::clear_ensure();
	const char* name = jack_port_name(port);
	ErrorHandler::checkNotNullOrThrow(name, "Could not obtain port name");
	unsigned long flags = jack_port_flags(port);
...
tdap::RefArray<float> Port::get_buffer(jack_port_t* port,
	...
	if (port) {
		jack_default_audio_sample_t* buffer =
				(jack_default_audio_sample_t*) (jack_port_get_buffer(port,
						frames));
		if (buffer) {
...
jack_port_t* Port::create_port(jack_client_t* client,
	...
	ErrorHandler::clear_ensure();
	jack_port_t* port = jack_port_register(client, definition.name,
			definition.type(), definition.flags(), 0);
...
void Port::disconnect_port_all(jack_client_t* client,
		jack_port_t* port) {
	ErrorHandler::get_message_clear();
...
bool Port::try_disconnect_port_all(jack_client_t* client,
		jack_port_t* port, int* result) {
	ErrorHandler::get_message_clear();
	return ErrorHandler::returnIfZero(jack_port_disconnect(client, port),
			result);
}
...
Ports::Ports(const PortDefinitions& definitions) :
		definitions_(definitions), ports_(
				Value<size_t>::max(1, definitions_.portCount()),
				definitions_.portCount()), registered_(false) {
}
