...
namespace speakerman {
    ...
    int JackClient::jackBufferSizeCallback(jack_nframes_t frames,
                                           void *client)
    {
        return client ?
               static_cast<JackClient *>(client)->onBufferSizeChange(frames) : 1;
    }

    int JackClient::jackSampleRateCallback(jack_nframes_t rate,
                                           void *client)
    {
        return client ?
               static_cast<JackClient *>(client)->onSampleRateChange(rate) : 1;
    }
    ...
    int JackClient::onXRun()
    {
        ...
        if (XRUNS_FOR_MEASUREMENT * MAX_ONE_OUT_OF / cyclesSinceFirstXrun) {
            cerr << "More than 1 out of " << MAX_ONE_OUT_OF
                 << " cycles Xrun: aborting" << endl;
        ...
        try {
            if (processor_->needsBufferSize()) {
                ErrorHandler::checkZeroOrThrow(
                        jack_set_buffer_size_callback(client_,
                                                      jackBufferSizeCallback, this),
                        "Set buffer size callback");
            }
            if (processor_->needsSampleRate()) {
                ErrorHandler::checkZeroOrThrow(
                        jack_set_sample_rate_callback(client_,
                                                      jackSampleRateCallback, this),
    ...
    speakerman::PortNames JackClient::portNames(jack_client_t *client,
                                                const char *namePattern, const char *typePattern, unsigned long flags)
    {
        const char **names = jack_get_ports(client, namePattern, typePattern,
                                            flags);
